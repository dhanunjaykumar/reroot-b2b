...
//        self.getBlockingReasons()
    ...
    override func viewDidLoad() {
        ...
        var tempBlockReason = BLOCK_REASON.init()
        ...
        self.getCustomBookingSetup()
        self.getDefaultBookingSetup()
...
//        popOver?.permittedArrowDirections = UIPopoverArrowDirection(rawValue: 0) //UIPopoverArrowDirection.any
    ...
    func getDefaultBookingSetup(){
        ...
        Alamofire.request(RRAPI.API_GET_DEFAULT_BOOKING_SETUP, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: headers).responseJSON{
            response in
    ...
    func getCustomBookingSetup(){
        ...
        Alamofire.request(RRAPI.API_GET_CUSTOM_BOOKING_SETUP, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: headers).responseJSON{
            response in
            switch response.result {
            case .success( _):
                print(response)
                do{
                    ...
                    HUD.hide()
                    let urlResult = try! JSONDecoder().decode(CUSTOM_BLOCK_REASONS.self, from: responseData)
                    print(urlResult)
...
//                        self.blockReasons.append(arrayaa)
    ...
    func blockOrReleaseSelectedUnit(_ sender: UIButton){
        ...
        //        obj.put(GSONParserConstants._ID, it._id)
        //        obj.put(GSONParserConstants.STATUS, it.status)
        //        obj.put("blockingReason", it.blockingReason)
        ...
        Alamofire.request(RRAPI.API_BLOCK_RELEASE_UNIT, method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: headers).responseJSON{
            response in
            switch response.result {
            case .success( _):
                print(response)
