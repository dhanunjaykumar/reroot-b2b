...
public final class DropDown: UIView {
	...
	*/
    public var customDataSource = [Dictionary<String,String>](){
        didSet{
            deselectRows(at: selectedRowIndices)
            reloadAllComponents()
        }
    }
    public var showingProjects : Bool = false
    public var showingBlocks : Bool = false
    public var showingTowers : Bool = false
    public var showingUnits : Bool = false
    public var showingVehicles : Bool = false
    var showingTrasportType : Bool = false
    public var showingDrivers : Bool = false
    public var showingBillingElements : Bool = false
    public var showingPremiumBillingElements : Bool = false
    public var showingBillingElementTypes : Bool = false
...
//            dataSource = localizationKeysDataSource.map { NSLocalizedString($0, comment: "") }
	...
	*/
	public convenience init(anchorView: AnchorView, selectionAction: SelectionClosure? = nil, dataSource: [Any] = [], topOffset: CGPoint? = nil, bottomOffset: CGPoint? = nil, cellConfiguration: ConfigurationClosure? = nil, cancelAction: Closure? = nil) {
		self.init(frame: .zero)

		self.anchorView = anchorView
		self.selectionAction = selectionAction
        self.dataSource = dataSource as! [Dictionary<String, String>]
...
//        }
	...
	public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(dataSource.count > 0){
//            let tempDict : Dictionary<String,Any> = dataSource[index] as! Dictionary<String,Any>
            
            if(showingTrasportType){
                let transportType = dataSource[index] as! String
                cell.optionLabel.text = transportType
                return cell
            }

            if(showingBillingElementTypes){
                let billingElementType = dataSource[index] as! String
                cell.optionLabel.text = billingElementType
                return cell
            }
            
            if(showingBillingElements){
                let billingInfo = dataSource[index] as! BILLING_INFO
                cell.optionLabel.text = billingInfo.name
                return cell
            }
            if(showingPremiumBillingElements){
                let billingInfo = dataSource[index] as! PREMIUM_BILLING_INFO
                cell.optionLabel.text = billingInfo.name
                return cell
            }
            
            if(showingVehicles){
                let vehicle = dataSource[index] as! Vehicle
                cell.optionLabel.text = vehicle.plateNo
                return cell
            }
            if(showingDrivers){
                let driver = dataSource[index] as! Driver
                cell.optionLabel.text = driver.name
                return cell
            }

            if(showingProjects){
                let project = dataSource[index] as! Project
                cell.optionLabel.text = project.name
                return cell
            }
            
            if(showingTowers){
                let tower = dataSource[index] as! Towers
                cell.optionLabel.text = tower.name
                return cell
            }
            if(showingBlocks){
                
                let block = dataSource[index] as! Blocks
                cell.optionLabel.text = block.name
                return cell
            }
            
            if(showingUnits){
                let unit = dataSource[index] as! Units
                cell.optionLabel.text = String(format: "%d (%@)", unit.unitIndex,unit.description1!)
                return cell
                //(tempUnit.unitNo?.index)!,tempUnit.description!))
            }
            
            
//            print(tempDict.keys.count)
//            print(tempDict.keys)
//
...
//            let keys : [String] = tempDict.keys
//            cell.optionLabel.text = "test" //keys[0]
...
//            }
//            else {
//                let tempDict : Dictionary<String,String>  = customDataSource[index]
//                let keys : [String] = tempDict.keys as! [String]
//                cell.optionLabel.text = keys[0]
//                customCellConfiguration?(index, keys[index], cell)
//
//            }
//        }
