...
//    @IBOutlet var heightOfStatusInfoView: NSLayoutConstraint!
    ...
    var statusChangeView : StatusChangeView!
...
//        changeButton.layer.cornerRadius = 8
    ...
    @objc func popControllers(){
        ...
        if(selectedProspectRow < 0 && isLongPress == false){
            self.navigationController?.popViewController(animated: true)
        }
        self.isLongPress = false
        if(statusChangeView != nil)
        {
...
//                print(response)
                ...
                do{
                    ...
                    DispatchQueue.main.async {
                        if(self.tableViewDataSourceArray.count > 0){
                            self.title = "Offers(\(self.tableViewDataSourceArray.count))"
                        }
                        else{
                            self.title = "Offers"
                        }
    ...
    @objc func showStatusChangeView(_ sender : UILongPressGestureRecognizer){
        ...
        if sender.state == .began{
            ...
            if(tabID != 2){
                self.statusChangeView.HLineView.isHidden = true
